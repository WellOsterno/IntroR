...
caminho <- "E:/Minicursos/IntroR/bases/base_pop.xlsx"
...
O caminho é o que identifica o arquivo que precisamos inserir. Neste caso estamos usando um formato específico por termos baixado todo o minicurso online. Para identificar o arquivo no seu computador, insira algo como:
...
1.  Crie dois ou mais objetos que identifiquem número (`a <- 1`) e faça comparações entre os objetos numéricos. Além disso, compare os objetos com número de fato, como: `a == 2`.

2.  Crie dois ou mais objetos que identifiquem palavras e faça pelo menos duas comparações entre elas e duas transformações à sua escolha.

3.  Crie uma pequena base de dados em um formato longo. Lembre-se que a função Data.Frame() permite essa criação no R e que o helper do R () pode ajudar com a estrutura.

4.  Leia uma documentação de pacote e uma documentação de função. Não precisa ler tudo, pois normalmente são documentos extensos, mas leia o início, que descreve o que é e o que faz o pacote/função. Há várias exemplos de pacote e funções usadas acima e na aula anterior, mas aqui estão algumas sugestões:
    ...
    2.  Funções:
        ...
        - [check_heaping_whipple](https://timriffe.github.io/DemoTools/reference/check_heaping_whipple.html)

        - [ggplot](https://www.rdocumentation.org/packages/ggplot2/versions/0.9.0/topics/ggplot)
